...
export default function PropertyOverview() {
  ...
  if (!property) return <div className="page"><p>{t('pages.propertyOverview.notFound')}</p></div>;
  ...
  return (
    <div className="page">
      <div className="overview-header">
        ...
        <Link to="/properties" className="btn btn-secondary">{t('pages.propertyOverview.back')}</Link>
      </div>
      ...
      <div className="overview-grid">
        <div className="panel">
          <div className="panel-header">{t('pages.propertyOverview.keyData')}</div>
          <div className="panel-body">
            <dl className="overview-dl">
              <dt>{t('pages.propertyOverview.type')}</dt><dd>{property.property_type || '—'}</dd>
              <dt>{t('pages.propertyOverview.yearBuilt')}</dt><dd>{property.year_built || '—'}</dd>
              <dt>{t('pages.propertyOverview.livingArea')}</dt><dd>{property.living_area_sqm ? `${property.living_area_sqm} m²` : '—'}</dd>
              <dt>{t('pages.propertyOverview.usableArea')}</dt><dd>{property.usable_area_sqm ? `${property.usable_area_sqm} m²` : '—'}</dd>
              <dt>{t('pages.propertyOverview.plotArea')}</dt><dd>{property.plot_area_sqm ? `${property.plot_area_sqm} m²` : '—'}</dd>
              <dt>{t('pages.propertyOverview.purchasePrice')}</dt><dd>{property.purchase_price ? `${Number(property.purchase_price).toLocaleString('de-DE')} €` : '—'}</dd>
              <dt>{t('pages.propertyOverview.marketValue')}</dt><dd>{property.market_value ? `${Number(property.market_value).toLocaleString('de-DE')} €` : '—'}</dd>
            </dl>
          </div>
        </div>

        <div className="panel">
          <div className="panel-header">{t('pages.propertyOverview.rentalStatus')}</div>
          <div className="panel-body">
            <div className="overview-stats">
              <div className="overview-stat">
                <span className="overview-stat-value">{propertyUnits.length}</span>
                <span className="overview-stat-label">{t('pages.propertyOverview.units')}</span>
              </div>
              <div className="overview-stat">
                <span className="overview-stat-value">{occupied}/{propertyUnits.length}</span>
                <span className="overview-stat-label">{t('pages.propertyOverview.rented')}</span>
              </div>
              <div className="overview-stat">
                <span className="overview-stat-value">{occupancyRate}%</span>
                <span className="overview-stat-label">{t('pages.propertyOverview.occupancy')}</span>
              </div>
              <div className="overview-stat">
                <span className="overview-stat-value">{totalRent.toLocaleString('de-DE')} €</span>
                <span className="overview-stat-label">{t('pages.propertyOverview.totalRentMonth')}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="panel">
          ...
          <div className="panel-body">
            {activeContracts.length === 0 ? (
              <p className="empty-text">{t('pages.propertyOverview.noActiveContracts')}</p>
            ) : (
        ...
        <div className="panel">
          ...
          <div className="panel-body">
            {insurances.length === 0 ? (
              <p className="empty-text">{t('pages.propertyOverview.noInsurances')}</p>
            ) : (
              <ul className="overview-list">
                {insurances.map(ins => (
                  <li key={ins.id}>
